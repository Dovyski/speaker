...
`<ClaudeDir>ttention\`, merges the three hook entries (`Stop`, `PostToolUse`,
`SessionStart`) into every settings file it was pointed at, removes the legacy
scheduled task if it is still registered, and prints what it did and what is
left to do by hand.
