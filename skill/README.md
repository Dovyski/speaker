...
`speak_exe/` is the skill that teaches a coding agent to drive `speak.exe`.
...
New-Item -ItemType Junction `
  -Path "$env:USERPROFILE\.claude\skills\speak_exe" `
  -Target "C:\Dev\www\claude-speak\skill\speak_exe"
```

Use `.claude/skills/speak_exe` inside a project instead if only that project
